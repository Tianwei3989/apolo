...
from model.optimization import RAdam
...
def main():
    ...
    parser.add_argument(
        "--save_name", type=str, help="save name for training."
    )
    parser.add_argument(
        "--in_memory",
        default=False,
    ...
    parser.add_argument(
        "--w_clf",
        default=False,
        help="training with the emotion classification loss or not",
    )
    parser.add_argument(
        "--freeze",
        default=-1,
        type=int,
        help="till which layer of textual stream of model need to fixed.",
    )
    parser.add_argument(
        "--vision_scratch",
        action="store_true",
    ...
    parser.add_argument(
        "--resume_file", default="", type=str, help="Resume from checkpoint, should be XXX.tar"
    )
    parser.add_argument(
        "--dynamic_attention",
        action="store_true",
    ...
    for epochId in tqdm(range(start_epoch, args.num_train_epochs), desc="Epoch"):
        ...
        for step in range(median_num_iter):
            ...
            if (
                step % (10 * args.gradient_accumulation_steps) == 0
                and step != 0
                and default_gpu
            ):
                tbLogger.showLossTrain()

            # decided whether to evaluate
            for task_id in task_ids:
